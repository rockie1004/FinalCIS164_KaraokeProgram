...
const bool DEBUGUSEFULTHINGS = false;
const int MIN_UNIQUE_ID = 100000;
...
bool openArtFileIn(fstream &file, string artInfo)
{
	bool fileStatus = false;
	//ArtFile.open("artwork.txt", ios::out | ios::in);
	/*if (ArtFile.fail())
	{
		fileStatus = false;
	}
...
/*Johannes's contribution 10/13. Commented out because does not compile. Sent him a request on 10/14 for tested and working code. 
Amy created int FindMaxUniqueID(fstream& inputReader) above instead.*/
/*const ID_SIZE = 6;
int highestID(fstream, int);

int highestID(file, idSize)
{
	int id = 0;
	while (file)
	{
		getline(file, fullid);
		while (fullid.size() > ID_SIZE)
		{
			fullid.pop_back();
		}
		if (stoi(fullid) > id)
